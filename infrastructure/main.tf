...
module "monitoring_rds" {
  ...
  rds_identifier       = var.monitoring_rds_identifier
  postgres_db_name     = var.monitoring_postgres_db_name
  postgres_db_username = var.monitoring_postgres_db_username
  postgres_db_password = var.monitoring_postgres_db_password
}
...
resource "aws_security_group" "mlflow_server_sg" {
  ...
  # Allow MLflow server traffic from any source
  ingress {
    from_port   = 5000
    to_port     = 5000
    protocol    = "tcp"
...
resource "aws_security_group" "mlflow_rds_sg" {
  ...
  # Allow PostgreSQL traffic from any source
  ingress {
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
...
resource "aws_security_group" "adminer_sg" {
  ...
  # Allow Adminer traffic from any source
  ingress {
    from_port   = 8081
    to_port     = 8081
    protocol    = "tcp"
...
resource "aws_security_group" "grafana_sg" {
  ...
  # Allow Grafana traffic from any source
  ingress {
    from_port   = 3000
    to_port     = 3000
    protocol    = "tcp"
